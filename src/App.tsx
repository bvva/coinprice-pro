...
import { Loader2, Wifi, WifiOff, AlertCircle, Heart, Grid } from 'lucide-react';
import { Header } from './components/Header';
import { CryptoCard } from './components/CryptoCard';
import { FullscreenPrice } from './components/FullscreenPrice';
import { NewsPanel } from './components/NewsPanel';
import { GridFullscreenMode } from './components/GridFullscreenMode';
...
function App() {
  ...
  // 自选列表状态
  const [activeTab, setActiveTab] = useState<'all' | 'favorites' | 'news'>('all');
  ...
  const [gridFullscreenMode, setGridFullscreenMode] = useState(false);
  const [gridSelectedSymbols, setGridSelectedSymbols] = useState<string[]>([
    'BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'XRPUSDT', 'SOLUSDT', 'ADAUSDT', 'DOGEUSDT', 'AVAXUSDT', 'TRXUSDT'
  ]);
  ...
  const { news, loading: newsLoading, error: newsError, refreshNews } = useNews(30000);
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 transition-colors duration-300 ${isDark ? 'dark' : ''}`}>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-4">
              {/* 分格全屏按钮 */}
              <button
                onClick={() => setGridFullscreenMode(true)}
                className="flex items-center space-x-2 px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 rounded-lg border border-blue-400/30 transition-colors"
                title={language === 'zh' ? '分格全屏模式' : 'Grid Fullscreen Mode'}
              >
                <Grid className="w-4 h-4 text-blue-400" />
                <span className="text-sm font-medium text-blue-400">
                  {language === 'zh' ? '分格全屏' : 'Grid Mode'}
                </span>
              </button>
          ...
          <div className="flex space-x-1 p-1 glass-effect rounded-lg">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'news'
                ? 'bg-primary-500 text-white shadow-lg'
                : 'text-gray-600 dark:text-gray-300 hover:bg-white/50 dark:hover:bg-white/10'
                }`}
            >
              {language === 'zh' ? '快讯' : 'News'} ({news.length})
            </button>
          </div>
        </motion.div>
      ...
      <FullscreenPrice
        ...
        newsError={newsError}
        onNewsRefresh={refreshNews}
      />

      {/* 分格全屏模式 */}
      <GridFullscreenMode
        isOpen={gridFullscreenMode}
        onClose={() => setGridFullscreenMode(false)}
        cryptoData={cryptoData}
        language={language}
        selectedSymbols={gridSelectedSymbols}
        onSymbolsChange={setGridSelectedSymbols}
        fullscreenMode={fullscreenMode}
        fullscreenBackground={fullscreenBackground}
        news={news}
        newsLoading={newsLoading}
